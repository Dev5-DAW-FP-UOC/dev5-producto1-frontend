    ...
    cont.innerHTML += `
      <div class="vol-item card mb-3">
        <div class="card-body p-3">
          ...
          <!-- VISTA ESCRITORIO: todo en línea -->
...
// ---- Añadir, editar voluntariado ----

let editandoId = null; // null para alta, id para editar

// Mostrar modal vacío (alta)
...
// Mostrar modal con datos (editar)
...
document.getElementById('formVoluntariado').addEventListener('submit', function(e) {
  ...
  // Recoge datos
  const v = {
    id: editandoId ? editandoId : Date.now(), // Simple id
  ...
  if (editandoId) {
    // Editar existente
    const idx = voluntariados.findIndex(x => x.id === editandoId);
    voluntariados[idx] = v;
  } else {
    // Alta
    voluntariados.push(v);
  }
...
function borrarVoluntariado(id) {
  if (confirm('¿Seguro que quieres borrar este voluntariado?')) {
    voluntariados = voluntariados.filter(v => Number(v.id) !== Number(id));
    renderVoluntariados();
  }
}
